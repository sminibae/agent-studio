...
이 문서는 구현·검증 절차를 정의한다. 프로젝트 골격의 패키지·Makefile·CI와
프런트엔드 → API → PostgreSQL 실행 경계는 구현됐다. 이후 기능은 같은 명령
계약을 확장하고 해당 경계의 검증을 추가한다.
...
명령이 구현되지 않은 기능을 검증했다고 보고하지 않는다. 문서만 바꾸는 작업은
링크·상충 규칙·diff를 검토한다. 구현 단계에서는 바뀐 경계에 해당하는 실행
검증을 수행하고 실패/미실행을 명시한다.
