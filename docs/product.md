...
사용자는 서버에 접속하는 개발자 본인과 소수 팀원이다. 플랫폼 안에서 Agent를
조합하고 직접 실행한다. 각 사용자는 Python 편집기에서 Tool 함수를 자유롭게
작성하고 화면에서 자산·Setup·실험을 관리한다. 첫 모델 provider는 OpenAI다.
데이터는 사용자별로 분리하고 첫 완성본부터 허용된 복수 사용자의 동시 실행을
지원한다.
...
| 영역 | 제공할 동작 |
| --- | --- |
| 접근 | 브라우저 로그인, allowlist의 복수 팀원 동시 사용; 개인 데이터·환경 격리 |
