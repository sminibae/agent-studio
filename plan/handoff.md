...
기준: 2026-09-23. 02-2 사용자 Tool 계약을 `feat/user-tool-contract`의
`7a6e0af`에서 구현·검증하고 `4f7499e`로 `dev`에 `--no-ff` 병합했다. 현재 브랜치는
`dev`이며 다음은 **[02 실행 기술 검증](02_runtime_spikes.md)의 02-3: 격리·종료
공격 테스트**다. 다음 작업은 새 기능 브랜치를 만들어 시작한다.
...
| SDK spike | `openai-agents==0.22.2` 고정. fake model의 schema/description 격리·오류/retry/parallel/cancel/로컬 hook과 사용자 Tool snapshot 계약 검증 코드 존재 |
| 사용자 환경 spike | 읽기 전용 Linux venv에서 SDK/fake Tool 실행, SDK 누락·계약 drift 분류, 같은 Tool의 Setup별 description 동시 격리를 검증. 전체 공격 격리는 남음 |
...
`a1b9ef8`에서 다중 사용자 운영 설계가, `4f7499e`에서 사용자 Tool 계약이 `dev`에
병합됐다. 원격 fetch/push는 이번에 수행하지 않았으므로 원격의 최신 상태까지
확인하거나 갱신한 것은 아니다.
...
- [사용자 Tool 계약](../backend/src/agent_studio/spikes/user_tool_contract.py),
  [계약 테스트](../backend/tests/test_user_tool_contract_spike.py)
...
- `tool` 변수의 sync/async 함수와 SDK `FunctionTool`을 허용한다. 누락, 비호출 값,
  지원하지 않는 callable 객체는 안정된 오류로 거부한다.
- signature·annotation·docstring에서 나온 이름·description·입력 schema를 JSON
  snapshot으로 만들고 재평가 전체 일치를 검사한다. drift는
  `tool_contract_mismatch`로 실패한다.
- Setup description은 원본 Version Tool을 바꾸지 않고 실행 직전 복제본에 적용한다.
  컨테이너 두 개를 동시에 실행해 같은 원문의 description이 섞이지 않음을 확인했다.
- `UV_CACHE_DIR=/private/tmp/agent-studio-uv-cache make check`가 통과했다. backend
  40개/frontend 4개 테스트, lint·typecheck·OpenAPI drift·production build를
  포함한다. macOS Docker Desktop에서 SDK container smoke도 통과했다.

## 다음 구현 세션에서 할 일

1. `git status --short --branch`로 깨끗한 `dev`인지 확인하고
   `feat/runtime-isolation-attacks` 같은 새 기능 브랜치를 만든다. 의존성이 없거나
   달라졌으면 `make install`로 lockfile을 복원한다.
2. 02-3을 진행한다. 두 owner의 파일·venv·dotenv를 별도 mount하고 서비스
   credential, `.git`, Docker socket과 타 owner 파일 접근이 실패하는 공격 fixture를
   컨테이너 smoke에 추가한다.
3. timeout, 메모리, PID, stdout/stderr 상한과 자식 프로세스 강제 종료를 각각
   안정된 오류로 분류하고 종료 뒤 컨테이너/자식이 남지 않는지 확인한다. 설치 작업은
   새 venv revision만 만들고 기존 revision을 변경하지 않는지도 검증한다.
4. Prompt의 Case Run별 재계산/호출 retry 중 값 유지와 deadline·블로킹 sync Tool,
   출력/Trace 상한 테스트를 02-4에서 이어간다.
   SDK/도구 결과를 제품 상태·이벤트 DTO로 매핑하되 아직 없는 제품 DB 전체를
   한 번에 만들지 않는다. 세부 남은 작업은 02-3~02-5를 따른다.
...
다음 변경의 완료 기준은 **악성 사용자 코드가 다른 owner/서비스 파일과 credential에
접근하지 못하고, timeout·메모리·PID·출력 상한 및 자식 프로세스 종료가 재현 가능한
오류와 정리 결과로 검증되는 것**이다. 현재 02-1~02-2 통과만으로 02 전체 완료나
다중 사용자 격리 완료를 선언하지 않는다.
...
이번 2026-09-23 작업에서 `make check`(backend 40개/frontend 4개)와 macOS SDK
container smoke를 실행해 통과했다. 첫 sandbox 실행은 Docker socket 권한으로
실패해 승인된 외부 실행으로 재시도했고, 구현 중 발견한 SDK 누락 import 순서를
수정한 뒤 최종 smoke가 통과했다. integration/E2E와 기존 Prompt 전용 container
smoke는 변경 경계 밖이라 실행하지 않았다.
